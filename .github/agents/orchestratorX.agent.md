...
**运行时规程**：所有触发式流程（环境自检、MCP 降级、Bus Payload 校验、检查点回滚、Prompt 预处理、Status Report）已拆分为独立模块文件，位于 `.github/skills/orchestrator-playbook/modules/` 目录下。在执行对应操作前，你必须仅加载该操作触发的模块文件（参考 SKILL.md 索引表），严禁全量加载。
...
- `/rollback [迭代编号]`：回滚到指定的迭代检查点。执行前加载 `.github/skills/orchestrator-playbook/modules/03-checkpoint-rollback.md`。
- `/status`：快速摘取当前 hybrid 文档中已完成与未完成的工作项。执行前加载 `.github/skills/orchestrator-playbook/modules/05-status-report.md`。
...
4. **总线 Payload 校验 (Bus Payload Validation)**：在将 Payload 传递给下游前，加载 `.github/skills/orchestrator-playbook/modules/02-bus-payload.md`，按其中的校验规则执行。
...
- 执行动作：
   1. **首次进入工作流时**，加载 `.github/skills/orchestrator-playbook/modules/01-environment-init.md` 执行环境自检。
   ...
   4. coder 完成后，加载 `.github/skills/orchestrator-playbook/modules/02-bus-payload.md` 执行 **Bus Payload 校验**，加载 `.github/skills/orchestrator-playbook/modules/03-checkpoint-rollback.md` 执行 **检查点写入**，然后自动调用 `evaluatorX` 智能体。
   5. 检查 evaluator 的返回结果：
      - 如果结果包含 `PASS`：再次执行检查点写入（评估通过），向用户汇报实现与审核已完成，等待下一步指令。
      - 如果结果包含修改建议：加载 `.github/skills/orchestrator-playbook/modules/04-prompt-preprocess.md`，调用 `promptMasterX` 优化修复指令，然后传给 `coderX` 修复，继续进入 `evaluatorX`。
...
- **并行编排（`-parallel` 参数）**：如果指令中包含 `-parallel` 参数，在上述步骤 2（plannerX 确认 PRD）之后、步骤 3（首次调用 coderX）之前，插入并行调度流程：
   1. **任务分解**：加载 `.github/skills/orchestrator-playbook/modules/06-parallel-dispatch.md`，分析 hybrid 文档 Section 7 的 feature/AC 项，按「文件集互斥 + 无导入耦合 + AC 完整性」规则识别可并行任务组。输出分组方案并等待用户确认（yes/no/adjust）。
...
- 执行动作：
   1. **首次进入工作流时**，加载 `.github/skills/orchestrator-playbook/modules/01-environment-init.md` 执行环境自检。
   2. 加载 `.github/skills/orchestrator-playbook/modules/04-prompt-preprocess.md`，调用 `promptMasterX` 优化用户需求（若符合跳过规则则直传），然后调用 `coderX` 智能体开始局部实现。
   3. coder 完成后，加载 `.github/skills/orchestrator-playbook/modules/02-bus-payload.md` 执行 **Bus Payload 校验**，加载 `.github/skills/orchestrator-playbook/modules/03-checkpoint-rollback.md` 执行 **检查点写入**，然后调用 `evaluatorX` 智能体。
...
- 执行动作：
   1. 加载 `.github/skills/orchestrator-playbook/modules/04-prompt-preprocess.md`，调用 `promptMasterX` 优化用户需求（若符合跳过规则则直传），然后调用 `coderX` 智能体执行最小必要修改。
