...
1. **读取上游总线管道负载**：优先读取 codeX 在对话流中输出并交接来的"阶段成果负载"（明确完成了哪些功能、改了哪些文件）。利用这些信息进行**定向缩小审核范围**，而不是盲目全局遍历。
2. **读取 git diff**：结合上述负载范围，获取当前工作区未提交的相对变更（unstaged + staged）。
3. 如果 git diff 为空，检索最近一次 commit 的变更。
4. **搜索上下文文件**：对于 codeX 重点提示引用的模块、接口、类型，搜索工程中相关文件以获得完整上下文。
...
在将结构化的详细评估报告覆盖写入 hybrid 文档的 `9` 区块后，为了形成高效的高频迭代闭环，**在将控制权交还 codeX 之前，必须在当前的对话流中直接输出一份"评估总结总线负载 (Pipeline Payload)"** 给对方，包含：
1. **核心审核范围摘要**：本次校验对齐的主功能及代码片段。
2. **重点漏洞/驳回项提取**：从详细报表中提取 P0/P1 的核心问题（具体到文件+问题点）作为醒目摘要反馈给下游。
3. **下一步行动建议**：精确指引 codeX 在新一轮的迭代中第一步应该从哪个文件的哪段逻辑着手修复。
