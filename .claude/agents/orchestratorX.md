...
All subagent handoffs must follow the Bus Payload Schema (load module 02 before each handoff). Refer to `.claude/skills/orchestrator-playbook/modules/02-bus-payload.md` for validation rules. Ensure skill alignment, separation of concerns, and full context passing between agents.
...
- **Parallel (`-parallel`)**: After PRD confirmation, analyze task dependencies, create shadow docs, dispatch to parallel coderX+evaluatorX pairs, then merge results. Refer to `.claude/skills/orchestrator-playbook/modules/06-parallel-dispatch.md` for full dispatch rules.
