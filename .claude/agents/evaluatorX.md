...
## Execution Rules
- For every evaluation task, load and follow `.claude/skills/evaluator-prd-audit/SKILL.md`.
- **Threshold Detection and Compression**: After reading the spec document, you MUST load and invoke the `.claude/skills/auto-compress-hybrid/SKILL.md` skill, which defines a 4-level progressive compression strategy (L0 Healthy to L1 Light to L2 Standard to L3 Aggressive). The skill determines the appropriate compression level based on file size and line count.
